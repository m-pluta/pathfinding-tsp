...
for desired_size in (50, 100):
    OUTPUT_PATH = f"AISearchfile{str(desired_size).zfill(3)}.{num_cities}.txt"

    if desired_size > num_cities:
        print("Desired size is too big")
        exit(1)

    with open(OUTPUT_PATH, "w") as file:
        file.write(f"SIZE = {desired_size},\n")
        
        for i in range(desired_size - 1):
            els = dist_matrix[i][i+1:desired_size]
            file.write(",".join(map(str, els)) + ",\n")
        
        file.write(f"NOTE = This is a custom city set reduced from {input_file}")
