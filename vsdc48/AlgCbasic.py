...
def get_dist(pop: Population) -> Distribution:
    ...
    for individual in pop:
        # Invert the length to make smaller tours have a larger share and not smaller
        sum += (max_length - individual.length)
        dist.append(sum)
...
def pick_individual(population: Population, dist: Distribution) -> Individual:
    ...
    Args:
        ...
        dist (Distribution): The probability distribution

    Returns:
        Individual: Randomly selected individual
    """
    # Pick a random position in the distribution
    r = random.randint(0, dist[-1] - 1)
    
    # Identify which index the position is located at, and return the individual
...
def mutate(ind: Individual) -> None:
    ...
    i1, i2 = tuple(random.sample(range(num_cities), 2))
    
    # Perform the swap
    temp = ind.tour[i1]
    ind.tour[i1] = ind.tour[i2]
    ind.tour[i2] = temp
    ...
    def solve(self) -> None:
        ...
        while True:
            ...
            for _ in range(pop_size):
                ...
                x = pick_individual(population, dist)
                y = pick_individual(population, dist)
